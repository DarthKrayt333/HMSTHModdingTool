...
namespace HMSTHModdingTool.RDTB
{
    ...
    public static class RDTBBatchReplacer
    {
        ...
        // ═════════════════════════════
        // DUPLICATE-BATCH FLAG PATTERN
        // Per-vertex W-flags for batches
        // whose VIF block sizes match
        // MOUTH_BLOCK_SIZES. Preserves
        // the bone-weight bits that the
        // PS2 VU1 reads during animation
        // switching (whistle, talk, etc).
        // Without these exact flags the
        // mouth mesh jumps out of position
        // when the game switches between
        // duplicate UV-variant batches.
        // Works for ALL characters that
        // use this block layout — not
        // just BOY.
        // ═════════════════════════════
        static readonly
            Dictionary<(int, int),
    ...
    {(11,2), (0x00000000u, 0x3F800000u)},
        ...
        // Block-size signature that
        // identifies mouth/eye/blink
        // type duplicate batches.
        // When CompilePureTri produces
        // this exact pattern of block
        // sizes the MOUTH_FLAGS table
        // is used for vertex W-flags
        // instead of the generic
        // F_ZERO/F_ONE defaults.
        static readonly int[]
            MOUTH_BLOCK_SIZES =
            new int[]
            {
                ...
                = null)
        {
            ...
            // AUTO-DETECT DEFAULT:
            // BIG source RDTBs default
            // to MIRROR output (not BIG)
            // because MIRROR is the safe
            // modding format that keeps
            // all 14 slots pointing to
            // shared data instead of
            // requiring 3 separate LOD
            // chunks. This is easier for
            // game to load modded content
            // and prevents crashes when
            // vertex counts change.
            // Small sources still stay
            // SMALL. Mirror stays MIRROR.
            // Only when user explicitly
            // passes --big flag does the
            // output actually become BIG.
            if (outFmt ==
            ...
            // ═══════════════════════════════
            // AUTO-DETECT: SMALL RDTB MAPS
            // NEED ORIGINAL NORMALS COPIED
            //
            // Small RDTBs (like map batches
            // that aren't SRDB embedded)
            // usually rebuild with default
            // normals mode. Without the
            // "match" flag, origNormals
            // stays empty and their default
            // normals branch has nothing
            // to match against, leaving
            // vertices with generic (0,1,0)
            // normals which cause wrong
            // lighting/rendering on wooden
            // surfaces, walls, etc.
            //
            // ISOLATED FIX: Auto-populate
            // origNormals for SMALL sources
            // ONLY when normalsMode is not
            // already "match" and there is
            // no SRDB auto-scale active
            // (which handles its own scale
            // via autoScaleInvert path).
            //
            // Zero effect on:
            //   - BIG source RDTBs
            //   - MIRROR source RDTBs
            //   - SRDB embedded RDTBs
            //     (autoScaleInvert != 1.0)
            //   - Any source when normalsMode
            //     is already "match"
            //   - Any existing normals logic
            //     (only fills the dictionary,
            //     does not change how it is
            //     read downstream)
            // ═══════════════════════════════
            if (normalsMode != "match"
                && sourceIsSmall
                && autoScaleInvert == 1.0f
                && origNormals.Count == 0)
            {
                Console.ForegroundColor =
                    ConsoleColor.Cyan;
            ...
            // ═══════════════════════════════
            // DUP-FORCE: Duplicate batch group
            // normal forcing.
            //
            // Detects groups of batches that
            // share the same tex_id and have
            // identical vertex geometry but
            // different UVs (mouth/eye/blink
            // animation variants). When ANY
            // member of such a group has a
            // changed vertex count in its OBJ
            // vs the original RDTB, ALL members
            // are forced to use the group
            // leader's original normals.
            //
            // This prevents the "mouth jump"
            // bug where changing vertices on
            // one duplicate batch causes the
            // other UV-variants to render with
            // wrong bone-space normals during
            // animation transitions.
            //
            // Works for ALL characters and ALL
            // duplicate-batch groups — not just
            // BOY's mouth. Any NPC with the
            // same bone|tex duplicate structure
            ...
            {
                // Re-read chunk offsets on the
                // (possibly expanded) rdtbData
                var offsF =
                    ReadChunkOffsets(rdtbData);
                ...
                if (offsF.Count >= 9)
                {
                    ...
                    // Group batches by bone|tex.
                    // Each unique combination
                    // that has >= 2 members is
                    // a candidate duplicate group.
                    var groups =
                    ...
                    foreach (var kv in groups)
                    {
                        ...
                        // Check if ANY member
                        // has a DIFFERENT vertex
                        // count in its OBJ vs
                        // the original RDTB.
                        bool anyVcChanged = false;
                        foreach (int mem in
                            kv.Value)
                        {
                            if (!batchObjs
                        ...
                        // No vertex count change
                        // in any member — nothing
                        // to fix for this group.
                        if (!anyVcChanged)
                            continue;

                        // Expand group to include
                        // ALL batches with the
                        // same tex_id, even those
                        // with different bones.
                        // This handles cases like
                        // BOY's mouth where batches
                        // 26 and 29 have different
                        // bone indices (334, 432)
                        // from the main group
                        // bone (300) but all share
                        // tex_id = 4.
                        var expandedGroup =
                            new List<int>(kv.Value);
                        ...
                        // Force all non-leader
                        // members to use leader's
                        // original normals
                        foreach (int mem in
                            expandedGroup)
                        {
            ...
            foreach (var kv in batchObjs)
            {
                            ...
                            bi))
                {
                        ...
                        > 0)
                    {
                             ...
                             i++)
                        {
                            ...
                            // for small RDTB maps
                            // (where user adds new
                            // geometry like statues).
                            // For BIG/MIRROR sources
                            // (characters like BOY),
                            // ALWAYS copy the game's
                            // original normal —
                            // nearest-neighbor is
                ...
                {
                    // skipMouthFlags = true for
                    // the GROUP LEADER only.
                    // Leader's own flags come
                    // from its original OBJ #vw
                    // comments. Non-leaders use
                    // the MOUTH_FLAGS lookup so
                    // their W-bits match what the
                    // VU1 animation code expects.
                    bool skipMouth =
                        dupForcedBatches
                            .Contains(bi)
                        && bi == dupForcedBatches
            ...
            // ═══════════════════════
            // UV POST-PATCH
            // For batches that kept
            // original bytes (same tri
            // count), patch their UVs
            // directly in the output
            // file using the source
            // RDTB as reference for
            // byte positions.
            // ═══════════════════════
            PatchKeptBatchUvs(
                outRdtb, srcRdtb,
            ...
            // (standalone module)
            // ═══════════════════════
            // Before the scaler runs,
            // snapshot the exact VIF
            // bytes of every DUP-FORCED
            // batch from the current
            // output RDTB. The scaler
            // applies centroid-delta to
            // vertices which corrupts
            // duplicate-batch groups
            // (mouth/eye/blink texture-
            // swap frames) because their
            // vertex data was compiled
            // by CompilePureTri and does
            // not need positional
            // adjustment. After the
            // scaler runs we restore
            // those exact bytes so
            // dup-forced batches keep
            // their correct geometry.
            var dupSnapshots =
                new Dictionary<int,
                    byte[]>();

            if (dupForcedBatches.Count
                > 0)
            {
                byte[] preScale =
                    File.ReadAllBytes(
            ...
            // ═══════════════════════════════
            // AUTO-DETECT: SKIP SCALER WHEN
            // AUTO-SCALE INVERT IS ACTIVE
            //
            // If the extractor recorded an
            // Auto Scale value in _info.txt,
            // the autoScaleInvert path above
            // already corrected every batch's
            // vertex coordinates to game-
            // space. Calling the In-Place
            // Scaler on top would re-apply
            // centroid-delta based on the
            // already-corrected coordinates,
            // causing unmodded batches in
            // SRDB embedded RDTBs to shrink.
            //
            // This is isolated auto-detection:
            // only fires when auto-scale is
            // active. All other RDTBs (non-
            // SRDB, no auto-scale in info)
            // still get the scaler exactly
            // as before.
            // ═══════════════════════════════
            if (autoScaleInvert != 1.0f)
            {
            ...
            // Restore dup-forced batch
            // bytes exactly. The scaler
            // may have modified their
            // vertex coordinates via
            // centroid-delta which is
            // wrong for texture-swap
            // duplicate groups.
            if (dupSnapshots.Count > 0)
            {
                byte[] postScale =
                    File.ReadAllBytes(
                        outRdtb);
            ...
            bool skipMouthFlags = false)
        {
            ...
            // ── MOUTH/DUP PATTERN DETECT ──
            // If the compiled strip produces
            // blocks whose sizes exactly match
            // MOUTH_BLOCK_SIZES, activate the
            // hardcoded W-flag table. This
            // preserves the per-vertex bone-
            // weight bits that the PS2 VU1
            // animation system reads when
            // switching between UV-variant
            // duplicate batches (mouth open/
            // closed, eye blink, etc).
                    ...
                    MOUTH_BLOCK_SIZES.Length)
            {
                ...
                // ADDITIONAL CHECK:
                // Also require the OBJ
                // vertex count to match
                // the original mouth's
                // expanded vertex count.
                // The MOUTH_FLAGS table
                // was recorded from a mesh
                // with exactly 92 unique
                // vertex slots. If the new
                // OBJ has a different vert
                // count, the flag indices
                // will point to the wrong
                // vertices causing holes
                ...
                new MemoryStream())
            {
                     ...
                     bi < nBlocks; bi++)
                {
                         ...
                         j < vc; j++)
                    {
                        ...
                        uint vflag;
                        if (useMouthFlags &&
                            MOUTH_FLAGS.TryGetValue(
                                (bi, j), out var mfV))
                        {
                            vflag = mfV.vf;
                        }
                        else if (vi < vertFlags.Count)
                        {
                            vflag = vertFlags[vi];
                        }
                        else
                ...
                int nPtrs)
        {
            ...
            // Find next NON-NULL pointer
            // to determine batch end.
            // Small RDTB maps have
            // nPtrs > actualBatches with
            // trailing NULL pointer slots.
            // Without this fix, the batch's
            // end would be computed as 0
            // (from the NULL slot), leaving
            // samples empty and normals
            // getting written as zeros.
            ...
            for (int i = 0; i < 14; i++)
                slots[i] =
                    BitConverter.ToUInt32(
                        outData,
                        0x10 + i * 4);
            uint meshOff = slots[11];
            if (meshOff == 0 ||
                meshOff == 0xFFFFFFFF)
                return;

            int patchedDirect = 0;
            int patchedNearest = 0;
            int skipped = 0;

            foreach (var kv in batchObjs)
            {
                int bi = kv.Key;
                if (bi >= nPtrs) continue;
        ...
        // in a batch's VIF blocks
        // ═════════════════════════════
        static int CountPhysicalVerts(
            byte[] data,
            int start, int end)
        {
            int total = 0;
        ...
        // ═════════════════════════════
        // TYPE 1: DIRECT UV PATCH
        // OBJ UV count == physical vert
        // count. Write UV[i] to VIF
        // row[i] directly.
        // ═════════════════════════════
        static bool PatchUvsDirect(
            byte[] data,
            int start, int end,
            List<float[]> uvs)
        {
        ...
        // ═════════════════════════════
        // TYPE 2: NEAREST VERTEX UV PATCH
        // OBJ UV count != physical vert
        // count. For each physical VIF
        // vertex, find the nearest OBJ
        // vertex by position and use
        // its UV.
        // ═════════════════════════════
        static bool PatchUvsNearest(
            byte[] data,
            int start, int end,
            List<float[]> objVerts,
            List<float[]> objUvs)
        ...
        static float ComputeAvgUvDelta(
            byte[] data,
            int start, int end,
            List<float[]> objVerts,
            List<float[]> objUvs)
        {
            ...
            return (float)(totalDelta
                           / count);
        }

        static bool VerifyVertexOrder(
            byte[] data,
            int start, int end,
            List<float[]> objVerts)
        {
            // Check first few VIF vertices
            // against OBJ vertices in order.
            // If positions match within
            // epsilon, the ordering is
            // correct for direct UV patch.
            ...
            if (checked_ == 0) return false;

            // If most vertices match in
            // order, the OBJ was extracted
            // by our tool (Type 1).
            // If few match, Blender
            // reordered them (Type 2).
            float ratio =
        ...
        // ═════════════════════════════
        // COUNT OBJ VERTS
        // Quick vertex line counter
        // used by DUP-FORCE detection
        // to compare OBJ vert count
        // against original RDTB vert
        // count without full parsing.
        // ═════════════════════════════
